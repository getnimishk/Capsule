...
// â”€â”€ Wait for DeepSeek's textarea to mount
...
async function tryInjectContext() {
  ...
  if (!pending || pending.target !== "deepseek" || Date.now() - pending.ts > 300000) return;

  const input = await waitForDeepSeekInput(10);
  if (input) {
  ...
  showPendingContextBanner(pending, async () => {
    const el = await waitForDeepSeekInput(5);
    if (el) {
      injectIntoDeepSeek(el, pending.context);
      return true;
    }
    return false;
  });
...
// BLOCK B â€” DeepSeek conversation scraper
...
function ensureStyles() {
  ...
  s.id = "cc-styles";
  s.textContent = `
    /* â”€â”€ Export button: matches DeepSeek ds-toggle-button--md style â”€â”€ */
    #cc-ask-ai-btn {
...
function saveMessagesToStorage(messages) {
  ...
  if (typeof checkContextAndShowWarning === 'function') {
    checkContextAndShowWarning(messages, "deepseek");
  }
...
function parseDeepSeekApiConversation(data) {
  const list = data.chat_messages || data.messages || [];
  ...
  list.forEach(m => {
    ...
    if (role && text) {
      messages.push({
        ...
        content: text.trim(),
        format: "text",
        timestamp: new Date((m.create_time || Date.now() / 1000) * 1000).toISOString()
      });
    }
  });
  return messages;
...
window.addEventListener("message", (e) => {
  if (e.data && e.data.type === "Capsule_API_CAPTURE") {
    if (e.data.url.includes("api/v0/chat/")) {
      const messages = parseDeepSeekApiConversation(e.data.data);
      if (messages.length) {
