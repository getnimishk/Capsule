// injectors/kimi.js
// Runs on https://kimi.ai/*
// Capsule â€” Platform Support: Kimi AI

const PENDING_INJECT_KEY = "pending_context_inject";

// â”€â”€ BLOCK A: Wait for Kimi input â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
async function waitForKimiInput(maxRetries = 60) {
  for (let i = 0; i < maxRetries; i++) {
    const el = findKimiInput();
    if (el) return el;
...
function findKimiInput() {
  // Primary: contenteditable editor wrapper in Kimi
  const ce = document.querySelector('div[contenteditable="true"][class*="editor" i], div[contenteditable="true"]');
  if (ce && ce.offsetParent !== null) return ce;
  // Fallback 1: any textarea
  const ta = document.querySelector('textarea[class*="input" i], textarea');
...
function injectIntoKimi(el, context) {
  el.focus();
...
async function submitKimiInput() {
  for (let i = 0; i < 20; i++) {
...
async function tryInjectContext() {
  ...
  if (!pending || pending.target !== "kimi" || Date.now() - pending.ts > 300000) return;

  const input = await waitForKimiInput(10);
  if (input) {
    try { await chrome.storage.local.remove([PENDING_INJECT_KEY]); } catch (_) {}
    injectIntoKimi(input, pending.context);
    showBanner("\u2713 Context injected \u2014 review and send!");
    return;
  }

  showPendingContextBanner(pending, async () => {
    const el = await waitForKimiInput(5);
    if (el) {
      injectIntoKimi(el, pending.context);
      return true;
    }
    return false;
  });
...
function scrapeCurrentConversation() {
  ...
  // Try Kimi message class indicators (send-item / receive-item)
  const userEls = document.querySelectorAll('[class*="send-item"], [class*="user-msg"], [data-role="user"]');
  const aiEls   = document.querySelectorAll('[class*="receive-item"], [class*="ai-msg"], [data-role="assistant"]');
  ...
  if (userEls.length > 0 || aiEls.length > 0) {
    ...
    for (const { el, type } of all) {
      ...
      if (type === "assistant") {
        ...
        clone.querySelectorAll("button, svg, [aria-hidden], .citation, [class*='action' i]").forEach(n => n.remove());
  ...
  // Fallback: alternates
  document.querySelectorAll("[class*='message-item' i], article").forEach((el, idx) => {
    const text = el.innerText?.trim();
    if (text) {
      messages.push({ type: idx % 2 === 0 ? "user" : "assistant", content: text, format: "text", timestamp: now });
    }
  });
...
function downloadCurrentChat() {
  ...
  const data = { id: `scraped_${Date.now()}`, title, url: window.location.href, messages, savedAt: new Date().toISOString(), source: "kimi", version: 1 };
...
function copyCurrentChat() {
  ...
  const title = messages.find(m => m.type === "user")?.content?.slice(0, 60) || "conversation";
  const lines = [`[Kimi conversation: "${title}"]`, `[Copied: ${new Date().toLocaleString()}]`, ""];
  for (const msg of messages) lines.push(`${msg.type === "user" ? "User" : "Kimi"}: ${msg.content}`, "");
...
function sendFromThisPage(target) {
  ...
  const lines = [`[Context from Kimi conversation: "${title}"]`, `[Scraped: ${new Date().toLocaleString()}]`, ""];
  for (const msg of messages) lines.push(`${msg.type === "user" ? "User" : "Kimi"}: ${msg.content}`, "");
...
// â”€â”€ CC_AI_OPTIONS (all except kimi) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
const CC_AI_OPTIONS = [
  ...
  { id: "mistral",    label: "Mistral",    color: "#FF7000", bg: "rgba(255,112,0,0.12)",    svg: `<svg width="14" height="14" viewBox="0 0 24 24" fill="#FF7000"><rect x="2" y="2" width="5" height="5" rx="1"/><rect x="9.5" y="2" width="5" height="5" rx="1"/><rect x="17" y="2" width="5" height="5" rx="1"/><rect x="2" y="9.5" width="5" height="5" rx="1"/><rect x="17" y="9.5" width="5" height="5" rx="1"/><rect x="2" y="17" width="5" height="5" rx="1"/><rect x="9.5" y="17" width="5" height="5" rx="1"/><rect x="17" y="17" width="5" height="5" rx="1"/></svg>` },
];
...
function findKimiSlot() {
  // Look for bottom input wrap/toolbar containers
  const slot = document.querySelector('[class*="input-wrap" i], [class*="chat-input" i], [class*="editor-wrap" i]');
  if (slot) return slot;
  const inp = findKimiInput();
  return inp ? inp.parentElement : null;
}

function injectKimiButton() {
  if (document.getElementById("cc-ask-ai-btn")) return;
  ensureStyles();
  const slot = findKimiSlot();
  if (!slot) return;
...
function watchForButtonRemoval() {
  if (_observer) return;
  _observer = new MutationObserver(() => { if (!document.getElementById("cc-ask-ai-btn")) setTimeout(injectKimiButton, 300); });
...
function saveMessagesToStorage(messages) {
  ...
  if (typeof checkContextAndShowWarning === 'function') {
    checkContextAndShowWarning(messages, "kimi");
  }

  const title = messages.find(m => m.type === "user")?.content?.slice(0, 60) || "Kimi conversation";
  const convId = `kimi_${window.location.href}`;
  return new Promise(async (resolve) => {
    ...
    const entry = { id: convId, title, url: window.location.href, messages, fingerprint, savedAt: new Date().toISOString(), source: "kimi", version: 1 };
...
function parseKimiApiConversation(data) {
  const rawList = Array.isArray(data) ? data : (data.items || data.messages || data.data || []);
  ...
  rawList.forEach(m => {
    ...
    if (role && content) {
      const type = (role === 'user' || role === 'human') ? 'user' : 'assistant';
      messages.push({
        type: type,
...
window.addEventListener("message", (e) => {
  if (e.data && e.data.type === "Capsule_API_CAPTURE") {
    if (e.data.url.includes("api/chat/")) {
      const messages = parseKimiApiConversation(e.data.data);
      if (messages.length) {
