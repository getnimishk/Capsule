// injectors/grok.js
// Runs on https://x.com/i/grok*
// Capsule â€” Platform Support: Grok (xAI)

const PENDING_INJECT_KEY = "pending_context_inject";

// â”€â”€ BLOCK A: Wait for Grok input â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
async function waitForGrokInput(maxRetries = 60) {
...
function findGrokInput() {
  // 1. grok.com textarea
  const ta = document.querySelector('textarea[placeholder*="Grok"], textarea[placeholder*="Ask"], textarea');
  if (ta && ta.offsetParent !== null) return ta;

  // 2. x.com DraftJS contenteditable
  const ce1 = document.querySelector(
    'div[contenteditable="true"][data-testid="tweetTextarea_0"],' +
    'div[contenteditable="true"][data-testid="tweetTextInput_main"]'
  );
  if (ce1 && ce1.offsetParent !== null) return ce1;

  const ce2 = document.querySelector('.DraftEditor-root div[contenteditable="true"]');
  if (ce2 && ce2.offsetParent !== null) return ce2;

  const ce3 = document.querySelector('div[contenteditable="true"]');
  if (ce3 && ce3.offsetParent !== null) return ce3;

  return null;
}

// ── BLOCK B: Inject + submit ──────────────────────────────────────────────────
function injectIntoGrok(el, context) {
  el.focus();
...
async function submitGrokInput() {
  for (let i = 0; i < 20; i++) {
...
// ── BLOCK C: Pending context injection ────────────────────────────────────────
async function tryInjectContext() {
  ...
  if (!pending || pending.target !== "grok" || Date.now() - pending.ts > 300000) return;

  const input = await waitForGrokInput(10);
  if (input) {
    try { await chrome.storage.local.remove([PENDING_INJECT_KEY]); } catch (_) {}
    injectIntoGrok(input, pending.context);
    showBanner("\u2713 Context injected \u2014 review and send!");
    return;
  }

  showPendingContextBanner(pending, async () => {
    const el = await waitForGrokInput(5);
    if (el) {
      injectIntoGrok(el, pending.context);
      return true;
    }
    return false;
  });
...
function scrapeCurrentConversation() {
  ...
  // Try: Grok conversation turns labeled by test id
  const userEls = document.querySelectorAll('[data-testid*="UserMessage"], [data-testid*="human"]');
  const aiEls   = document.querySelectorAll('[data-testid*="GrokMessage"], [data-testid*="BotMessage"]');
  ...
  // Fallback: look for article elements (Twitter-style) and alternate
  const articles = [...document.querySelectorAll("article, [data-testid='cellInnerDiv']")];
  articles.forEach((el, idx) => {
    const tweetText = el.querySelector('[data-testid="tweetText"]');
    const text = (tweetText || el).innerText?.trim();
    if (text) {
...
function downloadCurrentChat() {
  ...
  const data = { id: `scraped_${Date.now()}`, title, url: window.location.href, messages, savedAt: new Date().toISOString(), source: "grok", version: 1 };
...
function copyCurrentChat() {
  ...
  const title = messages.find(m => m.type === "user")?.content?.slice(0, 60) || "conversation";
  const lines = [`[Grok conversation: "${title}"]`, `[Copied: ${new Date().toLocaleString()}]`, ""];
  for (const msg of messages) lines.push(`${msg.type === "user" ? "User" : "Grok"}: ${msg.content}`, "");
...
function sendFromThisPage(target) {
  ...
  const lines = [`[Context from Grok conversation: "${title}"]`, `[Scraped: ${new Date().toLocaleString()}]`, ""];
  for (const msg of messages) lines.push(`${msg.type === "user" ? "User" : "Grok"}: ${msg.content}`, "");
  ...
  const AI_URLS = {
    ...
    perplexity: "https://www.perplexity.ai/", mistral: "https://chat.mistral.ai/chat",
    kimi: "https://kimi.ai/",
  };
  try {
...
// â”€â”€ CC_AI_OPTIONS (all except grok) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
const CC_AI_OPTIONS = [
...
function watchForButtonRemoval() {
  if (_observer) return;
  _observer = new MutationObserver(() => { if (!document.getElementById("cc-ask-ai-btn")) setTimeout(injectGrokButton, 300); });
...
function saveMessagesToStorage(messages) {
  ...
  if (typeof checkContextAndShowWarning === 'function') {
    checkContextAndShowWarning(messages, "grok");
  }

  const title = messages.find(m => m.type === "user")?.content?.slice(0, 60) || "Grok conversation";
  const convId = `grok_${window.location.href}`;
  return new Promise(async (resolve) => {
    ...
    const entry = { id: convId, title, url: window.location.href, messages, fingerprint, savedAt: new Date().toISOString(), source: "grok", version: 1 };
...
window.addEventListener("message", (e) => {
  if (e.data && e.data.type === "Capsule_API_CAPTURE") {
    // Optional future expansion for Grok GraphQL parsing
  }
});
